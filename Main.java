...
class Calculator {
    public static String ask() {
        ...
        System.out.print("In: ");
        String input1 = calc.nextLine(); // the user input
            ...
            while (flag1) {
                ...
                for (int j = 0; j < strarr.size(); j++) { // checks for "^" first and does the operation
                    if (strarr.get(j).equals("^")) {
                        strarr.set(j,
                                Math.pow(Double.parseDouble(strarr.get(j - 1)), Double.parseDouble(strarr.get(j + 1)))
                                        + "");
                ...
                for (int j = 0; j < strarr.size(); j++) { // checks for "*" and "/" first and does the operation
                    if (strarr.get(j).equals("*") || strarr.get(j).equals("/")) {
                        if (strarr.get(j).equals("*")) {
                            strarr.set(j,
                                    Double.parseDouble(strarr.get(j - 1)) * Double.parseDouble(strarr.get(j + 1)) + "");
                        } else {
                            strarr.set(j,
                                    Double.parseDouble(strarr.get(j - 1)) / Double.parseDouble(strarr.get(j + 1)) + "");
                            if (Double.parseDouble(strarr.get(j - 1)) % Double.parseDouble(strarr.get(j + 1)) != 0) {
                                deci++;
                            }
                        }
                        strarr.remove(j - 1);
                ...
                if (!(deci > 0)) { // if no decimals in expression or result outputs as integer
